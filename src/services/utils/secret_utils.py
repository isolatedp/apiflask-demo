# src/services/utils/secret_utils.py
...
class SecretUtils:
    ...
    def generate_hash(raw_data):
        ...
        if isinstance(raw_data, str):
            return SecretUtils._generate_hash_for_string(raw_data)
    ...
    def _generate_hash_for_string(raw_data):
        ...
        # 預處理字串以符合 bcrypt 要求
        raw_data = SecretUtils._bcrypt_pre_process_for_string(raw_data)
        
        # 驗證字串是否符合 bcrypt 的長度限制
        if not SecretUtils._check_string_bcrypt_condition(raw_data):
            return False
    ...
    def verify_hash(raw_data, hash_data):
        ...
        """
        if isinstance(raw_data, str):
            return SecretUtils._verify_hash_for_string(raw_data, hash_data)
        elif isinstance(raw_data, (bool, int, float)):
            raw_data = str(raw_data)
            return SecretUtils._verify_hash_for_string(raw_data, hash_data)
        elif isinstance(raw_data, (list, dict)):
